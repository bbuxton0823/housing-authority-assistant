...
**Low-latency mode (default): ConversationRelay.** `PHONE_MODE=relay` answers calls with Twilio's `<ConversationRelay>`: streaming speech recognition, ElevenLabs TTS rendered by Twilio (`ttsProvider="ElevenLabs"`, voice set by `RELAY_VOICE_ID`), and barge-in (callers can interrupt). The backend serves the conversation over a WebSocket at `/webhooks/voice/relay`; "talk to a real person" ends the session and the action webhook `<Dial>`s `HOUSING_OFFICE_PHONE`. **Prerequisite (one-time, your decision):** accept the *Predictive and Generative AI/ML Features Addendum* in Twilio Console -> Voice -> Settings -> General; without it Relay calls won't connect. No ElevenLabs API key goes into Twilio - Twilio bundles ElevenLabs and bills it through your Twilio account. Your tunnel must support WebSockets (ngrok and cloudflared both do). If anything misbehaves, set `PHONE_MODE=gather` to fall back instantly.

**Fallback mode (`PHONE_MODE=gather`) - ElevenLabs via <Play>:** the backend renders every reply to MP3 with your ElevenLabs key (per-agent voices) and serves it to Twilio via `<Play>` - so the same voices callers hear on the web work on the phone. No ElevenLabs key goes into Twilio, and no ConversationRelay/AI-ML addendum is required. Requirements: `WEBHOOK_BASE_URL` must be set (Twilio fetches the audio from it) and `ELEVENLABS_API_KEY` valid; if synthesis ever fails the call automatically falls back to Twilio Polly. Set `PHONE_TTS=polly` to force the fallback. Generated audio is cached in `python-backend/voice_cache/` (gitignored). ConversationRelay (`ttsProvider="ElevenLabs"`) remains an optional future upgrade for lower latency, but it requires accepting Twilio's Predictive/Generative AI-ML Features Addendum - that's an agreement for you to accept (or not) in Voice -> Settings -> General; nothing in this app needs it.
