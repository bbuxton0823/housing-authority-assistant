...
# Force UTF-8 encoding early in the process
...
# Import main agents with Unicode sanitization
try:
    from main import (
        triage_agent,
        general_info_agent,
...
# Set up UTF-8 for JSON encoding
...
# Unicode sanitization function
def sanitize_unicode_for_openai(text: str) -> str:
    """Remove problematic Unicode characters that cause encoding issues with OpenAI SDK"""
    if not isinstance(text, str):
        return text
    
    # Replace bullet points with ASCII equivalent
    text = text.replace('•', '-')
    ...
    # Remove other potential problematic Unicode characters
    # Keep only ASCII and basic Unicode that works well
    try:
        text.encode('ascii')
        return text
    except UnicodeEncodeError:
        # If it contains non-ASCII, clean it more aggressively
        import unicodedata
...
def sanitize_agent_instructions(agent):
    """Sanitize all text content in an agent to prevent Unicode issues"""
    if hasattr(agent, 'instructions') and agent.instructions:
        agent.instructions = sanitize_unicode_for_openai(agent.instructions)
    
    if hasattr(agent, 'description') and agent.description:
        agent.description = sanitize_unicode_for_openai(agent.description)
        
    if hasattr(agent, 'handoff_description') and agent.handoff_description:
        agent.handoff_description = sanitize_unicode_for_openai(agent.handoff_description)
    
    return agent

# Apply sanitization to all agents if available
if AGENTS_AVAILABLE:
    triage_agent = sanitize_agent_instructions(triage_agent)
    general_info_agent = sanitize_agent_instructions(general_info_agent)
    inspection_agent = sanitize_agent_instructions(inspection_agent)
    landlord_services_agent = sanitize_agent_instructions(landlord_services_agent)
    hps_agent = sanitize_agent_instructions(hps_agent)
...
async def chat_endpoint(req: ChatRequest):
    """Chat endpoint with Unicode-safe OpenAI Agents SDK integration"""
    ...
    # Sanitize user input to prevent Unicode issues
    sanitized_message = sanitize_unicode_for_openai(req.message)
    ...
    state["input_items"].append({"content": sanitized_message, "role": "user"})
    old_context = state["context"].model_dump().copy()
    guardrail_checks: List[GuardrailCheck] = []

    try:
        # Run with explicit UTF-8 handling
        result = await Runner.run(current_agent, state["input_items"], context=state["context"])
    except InputGuardrailTripwireTriggered as e:
        failed = e.guardrail_result.guardrail
        gr_output = e.guardrail_result.output.output_info
        gr_reasoning = getattr(gr_output, "reasoning", "")
        gr_input = sanitized_message
    ...
    for item in result.new_items:
        if isinstance(item, MessageOutputItem):
            ...
            # Sanitize response text to ensure it's safe
            text = sanitize_unicode_for_openai(text)
...
async def synthesize_voice(text: str, agent: str = "Triage Agent"):
    ...
    # Sanitize text before voice synthesis
    sanitized_text = sanitize_unicode_for_openai(text)
    audio_data = voice_service.text_to_speech_base64(sanitized_text, agent)
    return {"audio_base64": audio_data}

if __name__ == "__main__":
    import uvicorn
    print("🏠 Housing Authority Assistant - Unicode Fixed")
    print("=" * 50)
