...
# TARGETED UNICODE FIX: Only fix specific problematic characters, not all non-ASCII
def fix_unicode_hyphens(text: str) -> str:
    """Replace only the specific Unicode characters that cause header encoding issues"""
    if not text:
        return text
    
    # Only replace the specific problematic Unicode characters
    fixed = text
    fixed = fixed.replace("‑", "-")    # non-breaking hyphen (\u2011) to regular hyphen
    fixed = fixed.replace("–", "-")    # en dash (\u2013) to hyphen  
    fixed = fixed.replace("—", "-")    # em dash (\u2014) to hyphen
    fixed = fixed.replace(""", "\"")   # left double quotation mark (\u201c)
...
# Fix environment variables with targeted approach
openai_key = os.environ.get('OPENAI_API_KEY', '')
if openai_key:
    fixed_key = fix_unicode_hyphens(openai_key)
    if fixed_key != openai_key:
        os.environ['OPENAI_API_KEY'] = fixed_key
        print(f"🔧 Fixed Unicode hyphens in OPENAI_API_KEY")

elevenlabs_key = os.environ.get('ELEVENLABS_API_KEY', '')
if elevenlabs_key:
    fixed_key = fix_unicode_hyphens(elevenlabs_key)
    if fixed_key != elevenlabs_key:
        os.environ['ELEVENLABS_API_KEY'] = fixed_key
        print(f"🔧 Fixed Unicode hyphens in ELEVENLABS_API_KEY")
...
# Load environment variables AFTER targeted Unicode fixes
load_dotenv()
...
try:
    ...
    # Disable tracing to reduce potential issues
    ...
    print("✅ OpenAI Agents loaded with targeted Unicode fix")
except Exception as e:
...
async def health_check():
    ...
    key_preview = f"{api_key[:15]}...{api_key[-6:]}" if len(api_key) > 20 else "Not set"
...
async def chat_endpoint(req: ChatRequest):
    """Real OpenAI Agents with targeted Unicode fixes"""
    ...
    try:
        # Run the real OpenAI Agents
        result = await Runner.run(current_agent, state["input_items"], context=state["context"])
        logger.info(f"✅ OpenAI Agents executed successfully for {current_agent.name}")
...
if __name__ == "__main__":
    import uvicorn
    print("🏠 Housing Authority Assistant - Real AI (Fixed API Key)")
    print("=" * 65)
    ...
    print("Unicode Fix: ✅ Targeted fix applied")
