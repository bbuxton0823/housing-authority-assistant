...
# Configure custom OpenAI client with UTF-8 support
try:
    import openai
    from agents import set_default_openai_client, set_tracing_disabled
    
    # Create custom httpx client with explicit UTF-8 encoding
    ...
    print("✅ OpenAI Agents loaded with custom UTF-8 client")
except Exception as e:
...
async def chat_endpoint(req: ChatRequest):
    """Real OpenAI Agents with custom UTF-8 HTTP client"""
    ...
    try:
        # Run the real OpenAI Agents with custom client
        result = await Runner.run(current_agent, state["input_items"], context=state["context"])
        logger.info("OpenAI Agents executed successfully with custom client")
        ...
        # If it's the Unicode error, provide specific feedback
        if "'ascii' codec can't encode character" in str(e):
            return ChatResponse(
                conversation_id=conversation_id,
                current_agent=current_agent.name,
                messages=[MessageResponse(content="I'm experiencing encoding issues. This appears to be a known OpenAI SDK issue on macOS. Please try again or contact support.", agent=current_agent.name)],
                events=[],
                context=state["context"].model_dump(),
                agents=_build_agents_list(),
                guardrails=[],
...
if __name__ == "__main__":
    import uvicorn
    print("🏠 Housing Authority Assistant - Custom OpenAI Client")
    print("=" * 60)
    ...
    print("Client: Custom UTF-8 httpx client")
